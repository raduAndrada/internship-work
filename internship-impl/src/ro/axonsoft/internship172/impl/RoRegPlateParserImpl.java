...
public class RoRegPlateParserImpl implements RoRegPlateParser, Serializable {
	...
	public RoRegPlateProperties parseRegistrationPlate(final String registrationPlate)
		...
		if (registrationPlate == null) throw new InvalidRoRegPlateException();
		...
		//stringul care ar trebui sa contina numele judetului poate fi eronat => numar de inmatriculare invalid
		try {
		...
		return ImtRoRegPlateProperties.builder()
		        .judet(jd)
		        .letters(theLetters)
		        .digits(digits)
		        .build();
