...
import ro.axonsoft.internship172.api.ImtVehicleOwnersProcessResult;
...
public class StreamVehicleOwnersProcessorImpl implements StreamVehicleOwnersProcessor {
    ...
    public StreamVehicleOwnersProcessorImpl(final RoRegPlateParser roRegPlateParser, final RoIdCardParser roIdCardParser,
    ...
    public void process(final InputStream ciCarRegNbInputStream, final OutputStream processResultOutputStream)
        ...
        final VehicleOwnersProcessResult streamResult = ImtVehicleOwnersProcessResult.builder()
                .metrics(metrics)
                .errors(recordsIterator.errors)
                .build();
    ...
    private class VehicleOwnerRecordIterator implements Iterator<VehicleOwnerRecord> {
         ...
         *             - citirea din fisier provoaca eroare IOException -- care
         *             nu e de tip RuntimeException si
         *             trebuie tratata. Pentru a nu trata eroarea in blocuri cu
         *             try catch , exista un wrapper care extinde
         *             RuntimeException si
         *             arunca o eroare de tip IOException
         *
         */
        private VehicleOwnerRecord readNextValidRecord() throws IOExceptionRuntimeWrapper {
            ...
            /**
             * Citirea din fisier, un string pentru linia curenta
             * daca apare o linie invalida arunca eroarea
             * IOExceptionRuntimeWrapper
             * daca linia e null atunci procesarea se incheie -> returneaza null
             * daca linia e goala -> citeste linia urmatoare valida
             */
            String linieAux;
            try {
            ...
            /**
             * Parsarea campurilor existe pe o linie
             * daca linie nu are cel putin 1 camp atunci este invalida si nu
             * trebuie parsata ->
             * se trece la linia urmatoare din fisierul de intrare
             */
            ...
            if (linie.length < 2 || linie[0].length() == 0 || linie[1].length() == 0) {
                ...
                /**
                 * Linia poate fi parsata atunci se initilizeaza un obiect cu
                 * proprietatiile unei
                 * carti de identitate si se verifica daca linia din fisier
                 * contine o carte de
                 * identitate valida
                 * Daca cartea de identitate este invalida se adauga o eroare in
                 * lista de erori
                 * si se reinitializeaza obiectul cu proprietatiile unei carti
                 * de identitate la null
                 * altfel obiectul este setate la valoarea proprietatiilor
                 * cartii de ID
                 */
                ...
                /**
                 * Se verifica daca data citita din fisier este sau nu valida
                 * constrangeriile sunt impuse de formatul datii si de valoarea
                 * acesteia
                 * apar erori de parsare si erori pentru ca data este invalida
                 */
                Date data = null;
                try {
                    data = Date.from(LocalDate.parse(linie[1]).atStartOfDay(ZoneId.systemDefault()).toInstant());

                    final Date tooOld = Date.from(LocalDate.parse("1899-01-01").atStartOfDay(ZoneId.systemDefault()).toInstant());
                 ...
                 * inmatriculare la null
                 * verificarea lungimii liniei inainte de parsare
                 * daca linia nu contine un numar de inmatriculare nu se
                 * incearca parsarea campului
                 * daca numarul este strain se reseteaza obiectul cu
                 * proprietatiile numarului la null
                 * altfel obiectul va contine proprietatiile numarului curent
                 */
                if (linie.length >= 3) {
                    try {
                 ...
                 * calculele finale ea
                 * trebuie sa contina proprietatiile unei carti de identitate
                 * romanesti si
                 * o data valida
                 */
                if (roIdCardProperties != null && data != null) {
                    return ImtVehicleOwnerRecord.builder()
                            .idCard(roIdCardProperties)
                            .regPlate(roRegPlateProperties)
                            .idCardIssueDate(data)
                            .build();
                } else {
                    return readNextValidRecord();
                }
    ...
    /**
     * @author intern
     * Clasa statica wrapper pentru IOException
     */
