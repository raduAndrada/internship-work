package ro.axonsoft.internship172.data;
...
public class DbVehicleOwnersProcessorImpl implements DbVehicleOwnersProcessor {
    /**
     * Se folosesc cele doua servicii - pentru extragere date de procesat si
     * pentru
     * punerea lor in baza de date
     */
    private static final Logger LOG = LoggerFactory.getLogger(DbVehicleOwnersProcessor.class);

    private VehicleOwnerService vehicleOwnersService;
    private ResultService resultService;
    /**
     * Refolosirea parserelor si a procesorului Rezultatul va fi obtinut de
     * acelasi
    ...
    public void process(final Long batchId) {
        ...
        /**
         * Procesarea se face cu obiectul vehicleOwnersRecordsIterable. Se cauta
         * numarul
         * de inregistrari din baza de date pentru a stii cum sa se citeasca
         * paginile se
         * parcurge fiecare pagina se adauga intr-un iterator Iteratorul este
         * transmis
         * ca argument pentru a fi procesat clasei wrapper imbricate in aceasta
         * clasa
         */
        Iterable<VehicleOwner> vehicleOwnersRecordsIterable = Lists.newArrayList();
        final Integer vehicleOwnersCount = vehicleOwnersService.countVehicleOwner();
        ...
        for (int startIndex = 0; startIndex < vehicleOwnersCount; startIndex += pageSize) {
            ...
            final List<VehicleOwner> vehicleOwnersList = Lists.newArrayList(
                    vehicleOwnersService.getVehicleOwnersPage(ImtPageCriteria.of(startIndex, pageSize, batchId)));
        ...
        metrics.getUnregCarsCountByJud().forEach((k, v) -> unregCarsCountByJud.add(ImtResultUnregCarsCountByJud
                .builder()
                .judet(Judet.valueOf(k))
                .unregCarsCount(v)
                .build()));

        final List<ResultError> resultErrors = Lists.newArrayList();

        recordsIterator.errors.forEach(element -> resultErrors
                .add(ImtResultError.builder()
                        .type(element.getType())
                        .vehicleOwnerId(element.getVehicleOwnerId())
                        .build()));

        final ResultMetrics resultMetrics = ImtResultMetrics.builder()
                .batchId(batchId)
                .resultMetricsId(null)
                .oddToEvenRatio(metrics.getOddToEvenRatio())
                .resultErrors(resultErrors)
                .unregCarsCountByJud(unregCarsCountByJud)
                .passedRegChangeDueDate(metrics.getPassedRegChangeDueDate())
                .resultProcessTime(new java.sql.Timestamp(LocalDateTime.now().atZone(ZoneId.systemDefault()).toInstant().toEpochMilli()))
                .build();

        LOG.info("Inserarea rezultatului cu errori " + resultMetrics.toString());
        try {
            resultService.insertResultMetricsWithErrors(resultMetrics, ImtErrorCriteria.of(), ImtResultUnregCarsCriteria.of());
    ...
    private class VehicleOwnerRecordIterator implements Iterator<VehicleOwnerRecord> {
        ...
        private VehicleOwnerRecord readNextValidRecord() {
            ...
            if (iterator.hasNext()) {
                final VehicleOwner vehicleOwner = iterator.next();
                LOG.info("Batch curent :" + vehicleOwner.getBatchId().toString());
                LOG.info("Elementul curent este :" + vehicleOwner.toString());
                final Long vehicleOwnerId = vehicleOwner.getVehicleOwnerId();
                /**
                 * Daca in tabela au fost inscrise valori cu null pentru cartea
                 * de identiate sau
                 * pentru data inmatricularii, inregistrarea este invalida
                 */
                if (vehicleOwner.getRoIdCard() == null || vehicleOwner.getIssueDate() == null) {
                    LOG.info("S-a citit o linie invalida");

                    errors.add(ImtResultError.builder()
                            .type(0)
                    ...
                    /**
                     * O inregistrare din baza de date poate fi parsata daca
                     * proprietatiile cartii
                     * de identitate pot fi procesate Se initializeza la null si
                     * daca parser-ul
                     * reuseste parsarea sunt trecute intr-un obiect cu
                     * proprietatiile identificate,
                     * daca parser-ul nu reusete sa faca parsarea, se seteaza
                     * obiectul la null
                     */
                    ...
                    try {
                        ...
                        roIdCardProperties = roIdCardParser.parseIdCard(vehicleOwner.getRoIdCard());
                        LOG.info("Cardul :" + vehicleOwner.getRoIdCard() + " a fost procesat cu succes");
                    } catch (final InvalidRoIdCardException e) {
                        LOG.info("Cardul :" + vehicleOwner.getRoIdCard() + " contine erori");
                    ...
                    /**
                     * Se verifica daca data din tabela este valida incercand se
                     * fie parsata Daca
                     * data nu este valida atunci va ramane setata pe null,
                     * altfel va fi initilizata
                     * la valoarea din tabela Datele din tabela sunt extrase sub
                     * forma de
                     * java.sql.Date si aplicatia are o constrangere de a se
                     * folosi java.util.Date
                     * de aceea se face o conversie intre cele doua tipuri de
                     * date Erorile care pot
                    ...
                    try {
                        ...
                        data = new Date(vehicleOwner.getIssueDate().getTime());
                    ...
                    /**
                     * Initializarea unui obiect cu proprietatiile unui numar de
                     * inmatriculare la
                     * null verificarea lungimii liniei inainte de parsare daca
                     * linia nu contine un
                     * numar de inmatriculare nu se incearca parsarea campului
                     * daca numarul este
                     * strain se reseteaza obiectul cu proprietatiile numarului
                     * la null altfel
                     * obiectul va contine proprietatiile numarului curent
                     */
                    try {
                        roRegPlateProperties = roRegPlateParser.parseRegistrationPlate(vehicleOwner.getRegPlate());
                        LOG.info("Procesarea numarului de inmatriculare:" + vehicleOwner.getRegPlate()
                    ...
                    /**
                     * Ca o linie sa fie valida, luata in considerare pentru
                     * calculele finale ea
                     * trebuie sa contina proprietatiile unei carti de
                     * identitate romanesti si o
                     * data valida
                     */
